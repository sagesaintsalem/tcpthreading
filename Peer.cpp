...
Peer::Peer(string port) : port(port), socket(io_ctx), ssl_ctx(ssl::context::tlsv12) {
    ssl_ctx.set_default_verify_paths();
    try {
        ssl_ctx.use_certificate_chain_file("tcpdomain.crt");  // Server certificate
...
void Peer::connectToSender(const string& host_ip) {  // Client function
    ...
    ssl_sock.async_handshake(ssl::stream_base::client, [this, &ssl_sock](const boost::system::error_code& error) {
        if (error) {
            ...
            cout << "Connected to host at on port " << port << ". Start chatting...\n";
